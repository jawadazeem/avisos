 ...
 *
 * <p>
 * Runtime work is performed on virtual threads so the edge process can keep
 * startup non-blocking while still running a resilient connection supervisor,
 * telemetry heartbeat loop, and watchdog.
 * </p>
 */
public class NodeRuntime {
    ...
    NodeRuntime(
        ...
        this.heartbeatService = Objects.requireNonNull(
                heartbeatService,
                "heartbeatService"
        );
        this.batteryProvider = Objects.requireNonNull(
                batteryProvider,
                "batteryProvider"
        );
        ...
        while (isOperational()) {
            try {
                ...
                if (batteryLevel <= LOW_BATTERY_THRESHOLD) {
                    log.warn(
                            "R-Team watchdog: low battery detected, level={}%",
                            batteryLevel
                    );
                }
                ...
                if (currentState == State.STARTING) {
                    log.warn(
                            "R-Team watchdog: MQTT connectivity not yet established"
                    );
                } else {
                    log.debug(
                            "R-Team watchdog: state={}, batteryLevel={}%",
                            currentState,
                            batteryLevel
                    );
                }
                ...
                log.warn(
                        "R-Team watchdog: hardware vitals check failed: {}",
                        e.getMessage()
                );
            }
    ...
    private Duration nextRetryDelay(Duration currentDelay) {
        ...
        return Duration.ofMillis(
                Math.min(nextMillis, maxRetryDelay.toMillis())
        );
    }
