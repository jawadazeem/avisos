...
public class PahoMqttProvider implements MqttProvider {
  ...
  public synchronized void connect() throws MqttException {
    ...
    MqttClient newClient =
        new MqttClient(mqttConfig.brokerUrl(), clientId, new MemoryPersistence());
    ...
    client = newClient;
    log.info("Connected MQTT client: clientId={}, brokerUrl={}", clientId, mqttConfig.brokerUrl());
  }
  ...
  public synchronized void publish(String topic, byte[] payload) {
    ...
    try {
      MqttMessage message = new MqttMessage(Objects.requireNonNull(payload, "payload"));
