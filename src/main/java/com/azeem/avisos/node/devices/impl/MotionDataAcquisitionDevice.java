...
public class MotionDataAcquisitionDevice extends BaseDataAcquisitionDevice {
    private final List<Subscriber> subscriberList = new ArrayList<>();
    private HardwareLink hardwareLink;
    private int signalStrength = -70;
    private int batteryLife = 80;

    public MotionDataAcquisitionDevice(UUID id, Logger logger, HardwareLink hardwareLink) {
        super(id, logger, hardwareLink);
    }

    public MotionDataAcquisitionDevice(Logger logger, HardwareLink hardwareLink) {
    ...
    public void performSelfCheck() {
        if (!ping()) {
            logger.log("CRITICAL: PING to Motion DataAcquisitionDevice (" + Id + ") Failed", LogLevel.CRITICAL);
        }

        if (signalStrength < -100) {
            logger.log("CRITICAL: Signal strength is very weak on Motion DataAcquisitionDevice (" + Id + ")", LogLevel.CRITICAL);
        }

        if (batteryLife <= 10) {
            logger.log("CRITICAL: Battery less than 10% on Motion DataAcquisitionDevice (" + Id + ")", LogLevel.CRITICAL);
        }
    }

    @Override
    public String toString() {
        return "MotionDataAcquisitionDevice{" +
