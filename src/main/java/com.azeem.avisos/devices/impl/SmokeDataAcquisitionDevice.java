...
public class SmokeDataAcquisitionDevice extends BaseDataAcquisitionDevice {
    private final List<Subscriber> subscriberList = new ArrayList<>();
    private int signalStrength = -70;
    private int batteryLife = 80;

    public SmokeDataAcquisitionDevice(UUID id, Logger logger, HardwareLink hardwareLink) {
        super(id, logger, hardwareLink);
    }

    public SmokeDataAcquisitionDevice(Logger logger, HardwareLink hardwareLink) {
    ...
    public DeviceType getDeviceType() {
        return DeviceType.SMOKE_DEVICE;
    }
    ...
    public void setSignalStrength(int signalStrength) {
        if (signalStrength > 0) {
            System.err.println("Signal strength must be greater than zero for a smoke device.");
        }
    ...
    public void performSelfCheck() {
        if (!ping()) {
            logger.log("CRITICAL: PING to Smoke DataAcquisitionDevice (" + Id + ") Failed", LogLevel.CRITICAL);
        }

        if (signalStrength < -100) {
            logger.log("CRITICAL: Signal strength is very weak on Smoke DataAcquisitionDevice (" + Id + ")", LogLevel.CRITICAL);
        }

        if (batteryLife <= 10) {
            logger.log("CRITICAL: Battery less than 10% on Smoke DataAcquisitionDevice (" + Id + ")", LogLevel.CRITICAL);
        }
    }

    @Override
    public String toString() {
        return "SmokeDataAcquisitionDevice{" +
