...
main() {
    ...
    if [ "$(basename "$DB_CLIENT")" = "sqlite3" ] && [ -n "$DB_KEY" ] && [ "${AVISOS_DB_ALLOW_SQLITE3:-false}" != "true" ]; then
        fail "Encrypted DB key is configured from ${DB_KEY_SOURCE:-unknown source}, but only sqlite3 was found. Install sqlcipher, set AVISOS_DB_CLIENT=sqlcipher, or set AVISOS_DB_ALLOW_SQLITE3=true if your sqlite3 build has SQLCipher support."
    fi
...
SQL
    ...
    log "Encryption key source: ${DB_KEY_SOURCE:-not configured}"
