...
class DashboardEventBroadcasterTest {
  ...
  void onAlarmCreated_shouldSendAlarmToTopic() {
    AlarmRecord alarm =
        new AlarmRecord(UUID.randomUUID(), UUID.randomUUID(), AlarmSeverity.CRITICAL,
            "knife detected", AlarmStatus.ACTIVE, LocalDateTime.now(), null);
  ...
  void onNodeHeartbeat_shouldSendNodeToTopic() {
    NodeRecord node =
        new NodeRecord(UUID.randomUUID(), "node-01", "MQTT_TELEMETRY_NODE",
            NodeStatus.RESPONSIVE, 92.5, LocalDateTime.now());
  ...
  void onVisionAnalysis_shouldSendResponseAndNodeIdToTopic() {
    ...
    VisionResponse response =
        new VisionResponse(true, List.of(new Prediction("person", 0.95, 10, 20, 200, 300)), 120,
            "GPU");
    ...
    verify(messagingTemplate)
        .convertAndSend("/topic/vision",
            Map.of("nodeId", nodeId, "response", response));
  }
}
