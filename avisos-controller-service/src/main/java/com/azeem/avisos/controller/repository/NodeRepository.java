...
public interface NodeRepository {
  ...
  default List<NodeRecord> getAllNodes() {
    return getAllNodeEntities().stream().map(NodeMapper::toDomain).toList();
  }

  @SqlUpdate(
      "INSERT OR REPLACE INTO nodes (uuid, name, type, status, battery_level, last_seen)"
  ...
  void saveNode(
      ...
      @Bind("batteryLevel") double batteryLevel);
