...
/**
 * High-velocity edge noise suppression tool for logging. Collapses repetitive error signatures into
 * a single summary log.
 */
public class DeduplicatingLogger {
  ...
  public void logError(UUID nodeId, String errorCode, String message) {
    ...
    if (count == 1) {
      log.error(
          "[{}] FIRST OCCURRENCE - Node: {}, Code: {}, Msg: {}",
          componentName,
          nodeId,
          errorCode,
          message);
      ...
      log.warn(
          "[{}] REPETITIVE ERROR - Node: {}, Code: {}, Count: {}",
          componentName,
          nodeId,
          errorCode,
          count);
