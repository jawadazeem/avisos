...
public class AuthService {
   ...
   *
   * @return true if authentication is successful, false if password verification fails, throws
  ...
  public boolean authenticate(String username, String password) {
    ...
    if (user.isEmpty()) {
      throw new UserDoesNotExistException(
          "The user by the username " + username + " doesn't exist. Please try another username.");
    }
    return argon2.verify(user.get().passwordHash(), password);
  }

  /**
   * Save a new user with the provided username and password. The password will be hashed before
   * storing.
   */
  ...
  public boolean removeUser(String username, String password) {
    ...
    if (user.isEmpty()) {
      throw new UserDoesNotExistException(
          "Failed to remove user. The user by the username "
              + username
              + " doesn't exist. Please try another username.");
    }
  ...
  /**
   * Check if there are any users in the system. This can be used to determine if the initial setup
   * is complete.
   *
